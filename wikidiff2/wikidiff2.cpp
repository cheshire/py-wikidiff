...
void line_explode(const char *text, std::vector<std::string> &lines)
{
	...
	for (; *ptr; ++ptr) {
		...
		for (; *(ptr2+1); ++ptr2){
			switch(*ptr2) {
				case '(':
					if (dns_type == '\0') {
						dns_type = '(';
						dns_level = 1;
					} else if (dns_type == '(') ++dns_level;
					break;
				case ')':
					if (dns_type == '(') --dns_level;
					break;
				case '[':
					if (dns_type == '\0') {
						dns_type = '[';
			...
			if (dns_level == 0) dns_type = '\0';

			if ( *ptr2 == '\n' || ( dns_type == '\0' &&  *ptr2 == '.' && !my_istext(*(ptr2+1)))) break;
		}

		lines.push_back(std::string(ptr, ptr2+1));
...
extern "C" const char *wikidiff2_do_diff(const char *text1, const char *text2, int num_lines_context)
{
	try {
...
extern "C" const char * wikidiff2_do_word_diff(const char * text1, const char * text2)
{
	try {
		std::string lines1 = std::string(text1);
		std::string lines2 = std::string(text2);
		std::string ret;

		ret.reserve(strlen(text1)+strlen(text2)+10000);

		print_worddiff(lines1, lines2, ret);
