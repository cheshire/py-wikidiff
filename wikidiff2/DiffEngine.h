		...
		const std::vector<T> & to_lines, Diff<T> & diff) 
{
	...
	while (xi < n_from || yi < n_to) {
		...
		//file << del.size() << ',' << add.size() << '\n';

		//changed by george - :
		if (del.size() && add.size()){
			double score[del.size()+1][add.size()+1];
			int dir[del.size()+1][add.size()+1];
			...
			for (int x = 0; x<del.size(); ++x) {
				for (int y = 0; y<add.size(); ++y) {
					score[x+1][y+1] = (double)score[x][y] + _dist((std::string)*del[x],(std::string)*add[y]);
				
					//file << (std::string)*del[x] << '\n' << (std::string)*add[y] << '\n';
					//file << _dist((std::string)*del[x], (std::string)*add[y]) << '\n';
					//file << "--------\n";

					dir[x+1][y+1] = 0;
					
					if (score[x][y+1]+SCOST < score[x+1][y+1]) {
						score[x+1][y+1] = score[x][y+1]+SCOST;
						dir[x+1][y+1] = -1;
					}
					if (score[x+1][y]+SCOST < score[x+1][y+1]) {
						score[x+1][y+1] = score[x+1][y] + SCOST;
						dir[x+1][y+1] = 1;
					}
				}
			}
			...
			while (path.size())
			{
				...
				switch(d) {
					case -1:
						diff.add_edit(DiffOp<T>(DiffOp<T>::del, 
							std::vector<const T *>(del.begin()+x, del.begin()+x+1), empty));
						++x;
						break;
					case 1:
						diff.add_edit(DiffOp<T>(DiffOp<T>::add, empty, 
							std::vector<const T *>(add.begin()+y, add.begin()+y+1)));
						++y;
						break;
					case 0:
						diff.add_edit(DiffOp<T>(DiffOp<T>::change,
							std::vector<const T*>(del.begin()+x, del.begin()+x+1),
							std::vector<const T*>(add.begin()+y, add.begin()+y+1)));
						++x, ++y;
						break;
				}
			}
			// to make things work, comment uncommented and uncomment commented
